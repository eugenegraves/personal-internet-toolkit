<script lang="ts">
    ...
    onMount(() => {
        const updateDom = () => {
            domCount = document.querySelectorAll('*').length.toLocaleString();
        };
        updateDom();
        ...
        const updateHeap = () => {
            ...
            if (perf.memory) {
                heap = (perf.memory.usedJSHeapSize / 1048576).toFixed(1) + ' MB';
            } else {
                heap = 'N/A';
            }
        };
...
<div class="perf-panel {collapsed ? 'perf-panel--collapsed' : ''}">
    <button class="perf-panel__toggle" title="Toggle Performance Panel" onclick={() => collapsed = !collapsed}>
        {collapsed ? '📊' : '⚡'}
    </button>
    <div class="perf-panel__body">
        ...
        <div class="perf-panel__row">
            ...
            <span class="perf-panel__value perf-panel__framework" style="color: {fwColor}">Svelte</span>
        </div>
        <div class="perf-panel__row">
